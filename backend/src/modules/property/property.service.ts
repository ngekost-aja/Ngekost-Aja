...
export class PropertyService {
  ...
  /**
   * Get property by ID with extended data
   */
  async getPropertyById(id: number): Promise<PropertyResponse> {
    ...
    const rooms = await this.repository.findRoomsByPropertyId(id);
    const occupiedRooms = rooms.filter((r) => r.status === 'occupied').length;
    const availableRooms = rooms.filter((r) => r.status === 'available').length;

    // Get facilities
    const facilities = await this.repository.findPropertyFacilities(id);
  ...
  /**
   * Get properties by owner ID with extended data
   */
  async getPropertiesByOwner(ownerId: number): Promise<PropertyResponse[]> {
    const properties = await this.repository.findProperties(
      { ownerId },
      0,
      100, // Get up to 100 properties for an owner
    );

    // Enhance each property with room counts and facilities
    const enhancedProperties = await Promise.all(
      properties.map(async (p) => {
        const rooms = await this.repository.findRoomsByPropertyId(p.id);
        ...
        const facilities = await this.repository.findPropertyFacilities(p.id);
  ...
  async addPropertyFacility(data: AddPropertyFacilityRequest): Promise<void> {
    await this.repository.createPropertyFacility(
      data.propertyId,
      data.facilityId,
    );
  }
